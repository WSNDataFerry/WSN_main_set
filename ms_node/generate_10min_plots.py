...
def plot_score_components(dfs):
    ...
    for i, (node, df) in enumerate(dfs.items()):
        ...
        ax.set_title(f"{node}")
        # auto-scale to data range so inter-node differences are visible
        ax.margins(y=0.1)
...
def plot_stellar_score(dfs):
    ...
    ax_main.set_ylabel("Smoothed STELLAR Score Ψ", fontsize=11)
    # auto-scale: zoom into actual data range to make differences visible
    ax_main.margins(y=0.1)
    ax_main.set_title(
        f"EWMA-smoothed Ψ (α=0.25) — {best_node} consistently highest → elected CH  [y-axis auto-scaled]",
        fontsize=10
    )
    ...
    if best_node:
        ...
        if not d_ch.empty:
            ...
            ax_raw.set_ylabel("Ψ (auto-scaled)", fontsize=11)
            ax_raw.margins(y=0.1)
            ax_raw.set_title(
...
def plot_dashboard(dfs):
    ...
    ax3.set_title("Final STELLAR Score Ψ (auto-scaled)")
    ax3.set_ylabel("Ψ")
    ax3.margins(y=0.1)
    ...
    ax6.set_title("Network Centrality κ (auto-scaled)")
    ax6.set_ylabel("κ")
    ax6.margins(y=0.1)
