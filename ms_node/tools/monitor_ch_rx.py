...
def monitor_node(idx, port):
    ...
    while not stop_event.is_set():
        ...
        # Role detection from STATUS line: "STATUS: State=CH, Role=CH" or "State=MEMBER"
        l = line.lower()
        if "state=ch" in l and "role=ch" in l:
            s["role"] = "CH"
        elif "state=member" in l:
            s["role"] = "MEMBER"
        elif "state=candidate" in l:
            s["role"] = "CANDIDATE"
